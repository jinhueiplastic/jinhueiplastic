...
// 訂單日期用共用的民國年月曆選擇器（initRocDatePicker，定義在 shared.js——合併區域表單、
// 統計頁查詢區間的起訖日期也是用同一個），選到的日期存在隱藏欄位 #order-date-input
// （西元 'YYYY-MM-DD'，給存檔邏輯讀），畫面上的按鈕文字跟月曆本身都是民國年格式。
let orderDatePicker = null;

function setOrderDate(isoDate) {
    orderDatePicker.setDate(isoDate);
}

function initOrderDateField() {
    const today = new Date();
    const todayIso = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`;
    orderDatePicker = initRocDatePicker('order-date', { initialIso: todayIso });
}
